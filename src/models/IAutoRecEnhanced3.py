...
class IAutoRecEnhanced3:
    ...
    def build_opt_model(self):
        ...
        if self.optimizer == 'Adam':
            # self.model.compile(optimizer=Adam(learning_rate=learning_rate), loss=loss)
            self.model.compile(optimizer=Adam(learning_rate=self.learning_rate), loss=IAutoRecEnhanced3.masked_rmse)
        else:
            raise Exception('Optimizer not implemented')
    ...
    def model_builder(self, hp):
        ...
        if self.optimizer == 'Adam':
            ...
            self.model.compile(optimizer=Adam(learning_rate=hp_learning_rate), loss=IAutoRecEnhanced3.masked_rmse)
        else:
            raise Exception('Optimizer not implemented')
    ...
    def fit(self, rating_mat, rating_mat2, batch_size=256, epochs=500, verbose=2):
        self.hist = self.model.fit(x=rating_mat, y=rating_mat2,
                                   ...
                                   callbacks=[IAutoRecEnhanced3.es, IAutoRecEnhanced3.mc])
